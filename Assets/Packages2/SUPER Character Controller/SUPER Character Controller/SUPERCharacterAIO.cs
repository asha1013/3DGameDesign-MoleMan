...
public class SUPERCharacterAIO : MonoBehaviour{
    ...
    [HideInInspector] public float recoilOffsetX = 0f; // Camera recoil offset (bypasses smoothing)
    [HideInInspector] public float baseAngleX = 0f; // Smoothed camera angle without recoil
    ...
    #endregion
    void Start(){
        // Early validation - prevents errors if component exists on multiple objects
        if(playerCamera == null){
            Debug.LogWarning($"SUPERCharacterAIO on {gameObject.name}: playerCamera not assigned, disabling component");
            enabled = false;
            return;
        }
        ...
        baseAngleX = playerCamera.transform.localEulerAngles.x;
        if(lockAndHideMouse){
        ...
        if(capsule == null){
            Debug.LogWarning($"SUPERCharacterAIO on {gameObject.name}: CapsuleCollider component missing, disabling component");
            enabled = false;
            return;
        }
    ...
    void RotateView(Vector2 yawPitchInput, float inputSensitivity, float cameraWeight){
        ...
        switch (viewInputMethods){
            ...
            case ViewInputModes.Traditional:{  
                ...
                switch(cameraPerspective){
                    case PerspectiveModes._1stPerson:{
                        // Use stored base angle to avoid compounding recoil offset
                        Vector2 targetAngles = ((Vector2.right*baseAngleX)+(Vector2.up*p_Rigidbody.rotation.eulerAngles.y));
                        ...
                        // Store smoothed base angle
                        baseAngleX = targetAngles.x;

                        // Apply recoil offset after smoothing (bypasses camera weight)
                        targetAngles.x += recoilOffsetX;
    ...
    void HeadbobCycleCalculator(){
        if(enableHeadbob){
            if(!isIdle && currentGroundInfo.isGettingGroundInfo && !isSliding){
                ...
                // one bob per footstep - use 4x multiplier for full cycle per 0.5 headbobCyclePosition increment
                // 20% horizontal, 80% vertical
                headbobCameraPosition.x = (((Mathf.Sin(Mathf.PI * (4*headbobCyclePosition + 0.5f)))*(headbobPower/50))*0.25f)*headbobWarmUp;
                headbobCameraPosition.y = ((Mathf.Abs((((Mathf.Sin(Mathf.PI * (4*headbobCyclePosition)))*1.6f))*(headbobPower/50)))*headbobWarmUp )+internalEyeHeight;
                // only tilt when moving sideways
                float sidewaysAmount = Mathf.Abs(MovInput.x);
                headbobCameraPosition.z = ((Mathf.Sin(Mathf.PI * (4*headbobCyclePosition))) * (ZTilt/3) * sidewaysAmount)*headbobWarmUp;
            }else{
    ...
    void MovePlayer(Vector3 Direction, float Speed){
       // GroundInfo gI = GetGroundInfo();
        ...
        //Air Control
            ...
            // Only apply air control forces if airControlFactor > 0
            if(airControlFactor > 0 && Direction.magnitude > 0){
            ...
            if(_2DVelocity.magnitude>(currentGroundSpeed/100)&& !isIdle){
                if(cameraPerspective == PerspectiveModes._1stPerson){
                    if((enableHeadbob ? headbobCyclePosition : Time.time) > StepCycle && currentGroundInfo.isGettingGroundInfo && !isSliding){
                        //print("Steped");
                        CallFootstepClip();
                        StepCycle = enableHeadbob ? (headbobCyclePosition+0.25f) : (Time.time+((stepTiming*_2DVelocityMag)*2));
                    }
                }else{
    ...
    public void CallFootstepClip(){
        // check for custom PlayerFootsteps script first
        PlayerFootsteps customFootsteps = GetComponent<PlayerFootsteps>();
        if(customFootsteps != null){
            customFootsteps.OnFootstep();
            return;
        }

        // fallback to SUPER's built-in footstep audio
